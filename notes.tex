...
When $X$ is an infinite-dimensional normed vector space, it is not true that all linear functionals on $X$ are bounded. In fact it is not immediately obvious that there are any bounded linear functionals on an arbitrary space $X$. In the rest of this section we present the fundamental result that every normed vector space $X$ has plenty of bounded linear functionals.
...
For example, every norm is a sublinear functional, and so is ever seminorm (that is, norm without the non-vanishing property). Another example is the upper Riemann integral, defined on the space of bounded functions on $[a,b]$.

\begin{thm}[Hahn--Banach]
  \label{thm:hb}
  Let $X$ be a vector space, $Y\leq X$ a subspace of $X$, and $p$ a sublinear functional on $X$. Then any linear functional $\phi$ on $Y$ such that $\phi\leq p$ extends to a linear functional $\hat\phi$ on $X$ such that $\hat\phi\leq p$.
\end{thm}

Before proving the Hahn--Banach theorem, we present the following set of corollaries which help confirm our claim that every normed vector space $X$ has ``plenty'' of bounded linear functionals.

\begin{cor}
  Let $X$ be a normed vector space, $Y\leq X$ a subspace of $X$.
  \begin{enumerate}
  \item Any bounded linear functional on $Y$ extends to a bounded linear functional on $X$.
  \item If $Y$ is closed and $z\notin Y$, then there exists a bounded linear functional on $X$ such that $\phi(Y)=0$ and $\phi(z)\neq0$.
...
\begin{proof}
  (a) Since $\phi$ is a bounded linear functional, there exists some $M$ such that $\phi(x)\leq M\|x\|$ for all $x\in X$. We may therefore apply the Hahn--Banach theorem with the sublinear functional $p(x)=M\|x\|$. Thus $\phi$ extends to a linear functional $\hat\phi$ on $X$ such that $\hat\phi(x)\leq M\|x\|$. In particular, $\hat\phi$ is bounded too.

  (b) Since $Y$ is closed, the value $\delta=\inf\set{\|y-z\|\mid y\in Y}$ is nonzero. Hence if we define $\phi$ on the space $Y+\RR z$ by $\phi(y+cz)=c\delta$, we have that $\phi(Y)=0$ and $\phi(z)\neq0$. Moreover $\phi$ is bounded because
  \[|\phi(y+cz)|=|c|\delta\;\leq|c|\cdot\|-y/c-z\|=\|y+cz\|
  \]
  Thus we can use (a) to extend $\phi$ and obtain the desired bounded linear functional on $X$.

  (c) If $x\neq x'$ then $x-x'\neq0$. Applying (b) with $Y=\{0\}$ we can find a bounded linear functional $\phi$ such that $\phi(x-x')\neq0$. It follows that $\phi(x)\neq\phi(x')$.
\end{proof}

We now return to the proof of the Hahn--Banach theorem.

\begin{proof}[Proof of Theorem \ref{thm:hb}]
  We begin by showing that we can find a proper extension of $\phi$. That is, for any $z\in X\setminus Y$ we can extend $\phi$ to a linear functional $\phi'$ on $Y\oplus\RR z$ satisfying $\phi'\leq p$. For this we will define
  \[\phi'(y+cz)=\phi(y)+c\phi'(z)
  \]
  where $\phi'(z)$ will be determined in a moment. We require that for all $x$ and all $c\in\RR$ we have
  \[\phi(y)+c\phi'(z)\leq p(y+cz)
  \]
  To isolate $\phi'(z)$ we must consider the cases of negative and positive values of $c$ separately:
  \begin{align*}
    \phi(y)+c\phi'(z)&\leq p(y+cz)\\
    \phi(y)-c\phi'(z)&\leq p(y-cz)
  \end{align*}
  Solving each for $\phi'(z)$ and substituting $y$ with $y/c$ in each one gives us two constraints:
  \begin{align*}
    (\forall y)\;\phi'(z)&\leq p(y+z)-\phi(y)\\
    (\forall y)\;\phi'(z)&\geq-p(y-z)+\phi(y)
  \end{align*}
  In order for the constraints to be satisfiable, it is sufficient to have for all $y_1,y_2\in Y$ that $-p(y_1-z)+\phi(y_1)\leq p(y_2+z)-\phi(y_2)$. And this is indeed the case, since
  \begin{align*}
    \phi(y_1)+\phi(y_2)&=\phi(y_1+y_2)\\
                       &\leq p(y_1+y_2)\\
                       &=p(y_1-z+y_2+z)\\
                       &\leq p(y_1-z)+p(y_2+z)
  \end{align*}
  Thus we can find a suitable value for $\phi'(z)$ and successfully extend $\phi$ to $\phi'$ as required. To complete the proof we wish to apply this step repeatedly. Since the number of steps will be uncountable in general, it is necessary to phrase our construction using the standard \emph{Zorn's lemma}: if $P$ is a partially ordered set and every chain of $P$ has an upper bound, then $P$ has a maximal element.

  Now let $P$ be the collection of linear functionals that extend $\phi$ and are $\leq p$. Then $P$ is linearly ordered by function extension. A chain $\mathcal C$ of functions always has an upper bound, namely the set-theoretic union $\bigcup\mathcal C$ of the members of the chain. Moreover the union will be $\leq p$ because each member of the chain is $\leq p$.

  Therefore we can apply Zorn's lemma to find an element $\hat\phi$ of $P$ which is maximal with respect to function extension. We claim moreover that the domain of $\hat\phi$ must be all of $X$. Indeed, if it were not then by our argument above we would be able to properly extend the domain of $\hat\phi$ to find a larger element $\phi'$ of $P$. This contradicts the maximality of $\hat\phi$, and completes the proof.
\end{proof}
