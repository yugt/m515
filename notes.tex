...
An abstract measure space will consist of a set $X$ together with a measure function $\mu$ which satisfies just the most fundamental of the nice properties we have worked to establish for Lebesgue measure. Of course the Lebesgue measure $m$ can only be applied to some of the subsets of $\RR^d$, so an abstract measure $\mu$ the can only be expected to be applied to some of the subsets of $X$. In the next sequence of definitions, we lay the setting for describing collections of measurable sets.
...
Of course, the collection of Lebesgue measurable sets is also a Boolean algebra, and the Lebesgue measure is a finitely additive measure on it. But as we have seen, these two possess much stronger properties.

\begin{defn}
  A \emph{$\sigma$-algebra} on $X$ is a collection of subsets of $X$ which contains the sets $\emptyset$ and $X$, and is closed under countable intersections, countable unions, and complements.
\end{defn}

Again, we have seen several key examples of $\sigma$-algebras on $\RR^d$: the collection of all Lebesgue measurable sets, and the collection of all Lebesgue null or conull sets. If $X$ is any set then the collection of all countable or co-countable subsets of $X$ forms a $\sigma$-algebra. If $X$ is any topological space then the collection of Borel subsets of $X$ forms a $\sigma$-algebra, where a set is said to be Borel if it is constructible from the open sets using countable intersections, countable unions, and complements.
...
\begin{defn}
  Let $X$ be any set, $\mathcal A$ a boolean algebra on $X$, and $\mu_0$ a finitely additive measure on $\mathcal A$. Then $\mu_0$ is said to be a \emph{premeasure} if for every sequence of pairwise disjoint sets $A_i\in\mathcal A$, if $\bigcup A_i\in\mathcal A$ too, then $\mu_0(\bigcup A_i)=\sum\mu(A_i)$.
\end{defn}

The condition does not quite say that $\mu_0$ is countably additive, but rather that it has the potential to be. This is confirmed by the following keystone result of the subject.
