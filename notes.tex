...
\begin{exerc}
  If $S$ is a bounded set of real numbers, the supremum $\sup(S)$ is the least upper bound of $S$, and the infemum $\inf(S)$ is the greatest lower bound of $S$. Show that $b=\sup(S)$ if and only if for all $\epsilon>0$, there exists $s\in S$ such that $b-s<\epsilon$. Formulate and prove the analogous statement for the infemum.
\end{exerc}
...
\begin{lem}
  ...
  \item For all $\epsilon>0$ there is an elementary set $E$ such that $m(E\triangle A)<\epsilon$.
  \end{itemize}
\end{lem}
...
\begin{exerc}[See Tao, Ex 1.1.5]
  Complete the proof of Lemma~\ref{lem:jordan-equiv}: $A$ is Jordan measurable iff for all $\epsilon>0$ there is an elementary set $E$ such that $m(E\triangle A)<\epsilon$.
\end{exerc}
...
\begin{thm}
  If $A$ is an elementary subset of $\RR^n$, then $m^*(A)$ agrees with the elementary measure $m_e(A)$.
\end{thm}

\begin{proof}
  It is clear that $m^*(A)\leq m_e(A)$, since $A$ is itself a union of boxes whose volumes sum to $m(A)$. Thus it remains only to show $m^*(A)\geq m_e(A)$. Appealing to the definition of $m^*(A)$, given any $\epsilon>0$ we can find boxes $B_i$ such that $A\subset\bigcup B_i$ and $\sum\vol(B_i)-m^*(A)<\epsilon$. Rearranging, this says $m^*(A)>\sum\vol(B_i)-\epsilon$. Now we would like to say that $\sum\vol(B_i)\geq m_e(A)$, but unfortunately the elementary measure is only \emph{finitely} subadditive.

  In order to proceed, let us temporarily assume that $A$ is closed and the $B_i$ are open. Then by the compactness of $A$, just finitely many of the $B_i$ are needed to cover $A$. Thus the argument of the previous paragraph works in this case!
  ...
  In order to assume that $A$ is closed, first write it as a finite union of disjoint boxes $C_1,\ldots C_k$. Shrinking each $C_i$ slightly, we can find a closed box $C'_i\subset C_i$ such that $m_e(C_i\setminus C'_i)<\epsilon/k$. Replacing $A$ with $\bigcup C'_i$ we obtain a closed set as desired.
\end{proof}
