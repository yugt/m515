...
\begin{defn}
  If $f=\sum_1^kc_i\chi_{E_i}$ is a simple function and $f\geq0$, then the \emph{simple integral} of $f$ is defined to be $\text{s}\int f=\sum_1^kc_im(E_i)$.
\end{defn}
...
\begin{prop}
  ...
  \begin{enumerate}
  \item (equivalence) if $f=g$ almost everywhere then $\text{s}\int f=\text{s}\int g$
  \item (monotonicity) if $f\leq g$ almost everywhere then $\text{s}\int f\leq\text{s}\int g$.
  \item (linearity) $\text{s}\int (cf)=c\cdot\text{s}\int f$, and $\text{s}\int (f+g)=\text{s}\int f+\text{s}\int g$.
  \end{enumerate}
\end{prop}
