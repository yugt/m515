...
The term measure has many English meanings. In this area of mathematics, ``measure'' is a number assigned to a set which represents its size. Of course the term ``size'' also has many meanings English meanings. Our concept of measure can accommodate many different types of sizes, for examples: length, area, volume, mass, and even probability. On the other hand, some other types of size are not usually associated with the mathematical concept of measure, for examples: cardinality, diameter, and density.
...
For simple sets, it is easy to decide what the measure should be. For example, the measure of the interval $[4,7]$ should be $3$ because that is its length. But for more complicated sets, the decision may not be so easy. If you have seen the construction of the Cantor set, think about how you would measure the length of that!

Thus we arrive at the ``Measure Problem,'' which asks whether it is possible to even find a function which adequately measures subsets of $\RR$. Of course it is necessary to decide what ``adequately'' means. The classical version of the Measure Problem proposed the three properties below. Formally it asked: Does there exist a measure function $m$ which assigns to each subset $A\subset\RR$ a value $m(A)\in[0,\infty]$ satisfying:
\begin{enumerate}
...
Perhaps surprisingly, no such measure function $m$ exists! While properties~(a)--(c) seem very natural, the three items actually turn out to be mutually inconsistent.
