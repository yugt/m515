...
\begin{exerc}[Tao, ex 1.3.25(ii)]
  Let $f$ be a measurable set with finite measure support. Show that for any $\epsilon>0$, there exists a measurable set $A$ such that $m(A)<\epsilon$ and $f$ is locally bounded outside of $A$. In other words, for every $n$ there exists $M$ such that for all $x\in[-n,n]^d\setminus A$ we have $f(x)\leq M$.
\end{exerc}
...
\begin{exerc}
  Show that a countable linear combination of measures is a measure.
\end{exerc}

\begin{exerc}
  Caratheodory property for Lebesgue measurable sets.
\end{exerc}
