...
\begin{proof}
  We first show that if $A$ lies in $\mathcal A$ then $A$ is $\mu^*$-measurable. For this, let $S$ be an arbitrary set and find sets $B_n\in\mathcal A$ such that $S\subset\bigcup B_n$ and $\mu^*(S)-\sum\mu_0(B_n)<\epsilon$. Then
  \begin{align*}
  ...
  Next we show that if $A$ lies in $\mathcal A$ then $\mu^*(A)=\mu_0(A)$. It is clear from the definition that $\mu^*(A)\leq\mu_0(A)$. To show that $\mu_0(A)\leq\mu^*(A)$, let $B_n\in\mathcal B_0$ be such that $A\subset\bigcup B_n$. We can assume without loss of generality that the $B_n$ are pairwise disjoint. Then using the axiom of a premeasure, we obtain
  \begin{align*}
...
\begin{proof}[Proof outline]
  ...
  It is clear from monotonicity that $\mu_F(\bigcup A_n)\geq\sum\mu_F(A_n)$, so it suffices to show that $\mu_F(\bigcup A_n)\leq\sum\mu_F(A_n)$. In order to achieve this, let us define $\mu_F$ on open intervals by letting $\mu_F((a,b))=\lim_{x\to b^-}F(x)-F(a)$, and similarly for closed intervals. Then using the right-continuity of $F$, we can pay the $\epsilon$ price to replace each $A_n$ with an open $U_n$ such that $A_n\subset A_n$ and replace the interval $I$ with a compact $K$ with $K\subset I$.

  Then by the Heine--Borel theorem, just finitely many of the $U_n$ suffice to cover $K$, and hence $\mu_F(K)\leq\sum\mu_F(A_n)$. It follows that $\mu_F(\bigcup A_n)\leq\sum\mu_F(A_n)-\epsilon$, and the result follows by taking $\epsilon\to0$.
\end{proof}
