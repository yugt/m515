...
Recall that a set $A$ is called Lebesgue measurable if it can be well-approximated from the outside by open sets. That is, if for all $\epsilon>0$ there is an open set $O$ such that $m^*(O\setminus A)<\epsilon$. In the last section we saw that open sets can be written as unions of boxes, and it turns out one may replace $O$ with a union of boxes in the definition.
...
\begin{proof}
  ...
  For countable unions, suppose that $A_n$ are Lebesgue measurable. Given $\epsilon>0$, find open sets $O_n$ such that $A_n\subset O_n$ and $m^*(O_n\setminus A_n)<\epsilon/2^n$. Then we have
  \begin{align*}
    m^*(\bigcup O_n\smallsetminus\bigcup A_n)
    &\leq m^*(\bigcup O_n\setminus A_n)\\
    &\leq \sum m^*(O_n\setminus A_n)\\
    &\leq \sum \epsilon/2^n=2\epsilon
  \end{align*}
  This shows that $\bigcup A_n$ is measurable.

  For closed sets, assume first that $A$ is closed and bounded, and thus compact. Using the outer regularity lemma we can find an open set $O$ such that $A\subset O$ and $m^*(O)-m^*(A)<\epsilon$. We wish to show that $m^*(O\setminus A)<\epsilon$ too. Since $O\setminus A$ is open, we can write it as an almost disjoint union of closed dyadic cubes $C_n$. Then $\bigcup_1^N C_n$ is compact and thus separated from the compact set $E$. By Lemma~\ref{lem:separated}, additivity holds for separated sets, so we have:
  \begin{align*}
  ...
  For complements, let $A$ be measurable and for each $n$ find an open set $O_n$ such that $A\subset O_n$ and $m^*(O_n\setminus A)<1/n$. Then we have $m^*(\bigcap O_n\setminus A)=0$. Meanwhile taking complements we have $\bigcup O_n^c\subset A^c$ and $m^*(A^c\setminus\bigcup O_n^c)=0$. In particular $A^c$ can be expressed as a union of the two measurable sets $\bigcup O_n^c$ and $A^c\setminus\bigcup O_n^c$. Thus $A^c$ is measurable too.
...
\begin{proof}
  ...
  Finally for general $E_n$, decompose $\RR^d$ into disjoint bounded cells $C_m$. Then $A_n=\bigcup_m A_n\cap C_m$. Now the sets $A_n\cap C_n$ are bounded, so applying the result for bounded sets twice we have:
  \begin{align*}
...
\begin{exerc}[Tao Ex 1.2.12]
  Suppose you know that $m$ satisfies $m(\emptyset)=0$ and the countable additivity property. Show that $m$ satisfies the monotonicity property and the countable subadditivity property (for measurable sets).
\end{exerc}
