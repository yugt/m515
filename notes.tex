...
%% Question: why not use countable unions of boxes in place of the open set O in this definition, too??
...
\begin{proof}
  The axioms (a) and (b) are both trivial, so it remains to prove only axiom (c). Let $E_n$ be arbitrary sets and let $\epsilon>0$ be given. From definition of Lebegue outer measure, for each $n$ we can find a sequence of boxes $B_i^n$ such that $E_n\subset\bigcup_iB_i^n$ and $\sum_im(B_i^n)<m^*(E_n)+\epsilon/2^n$.
...
In the previous section we introduced the Lebesgue outer measure $m^*$ and showed that it satisfies the axioms of an outer measure: empty set, monotonicity, and countable subadditivity. We are really interested in countable additivity, but we know this axiom cannot hold for all sets $E_n$. Instead we will 
