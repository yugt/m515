...
\begin{proof}
  ...
  Given $T,T'\in B(X,Y)$, we must show that the pointwise sum $T+T'$ lies in $B(X,Y)$ too.
  
  (b) It remains only to show that $B(X,Y)$ is complete.
...
To further the analogy with row vectors and inner products, some authors write $\langle\phi,x\rangle$ to denote $\phi(x)$.

By the results of this section, the dual space $X^*$ is always a Banach space with the operator norm. By the results of the previous section, the elements of the dual space are plentiful in the sense that they separate the points of $X$.

In fact in the statement of Hahn--Banach\ldots
...
\begin{proof}
  ...
  \[\hat x(\phi)=\phi(x)
  \]
  In the inner product notation introduced previously, this simply says $\langle\hat x,\phi\rangle=\langle\phi,x\rangle$. Clearly $\hat x$ is a function $X^*\to\RR$. Moreover $\hat x$ is a bounded operator because $|\hat x(\phi)|=|\phi(x)|\leq\|\phi\|\cdot\|x\|$. This calculation in fact shows that $\|\hat x\|\leq\|x\|$.

  Thus we have shown that the mapping $x\mapsto\hat x$ carries $X$ to $\hat X$. It is easy to see that it is a linear mapping and that it is one-to-one. To show that it preserves the norm, it remains only to show that $\|\hat x\|\geq\|x\|$.
\end{proof}

The above proposition may seem esoteric, but it has many uses. For example if we combine it with the fact that every dual space is complete, we can construct the completion of any normed vector space $X$ by embedding it inside $X^{**}$ and taking its closure.
