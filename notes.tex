...
It is important to note that for an infinite-dimensional Hilbert space, an orthonormal basis in the above definition is not a vector space basis in the true sense.
...
For another example, let $X=L^2[0,1]$. 


\begin{thm}[Parseval's identity]
  Let $X$ be a Hilbert space and $\{e_\alpha\}_{\alpha\in A}$ an orthonormal basis for $X$. Then for any $x\in X$, we have $\|x\|^2=\sum_{\alpha\in A}|\langle x,e_\alpha\rangle|^2$.
\end{thm}
...
One can interpret this result as saying that any Hilbert space $X$ looks remarkably like $\ell^2$. That is, each element $x\in X$ is determined by its vector of coefficients $\langle x,e_\alpha\rangle$.
