...
\begin{lem}
  Suppose that $E,F$ are finitely separated, that is, that $d(E,F)=\inf\set{d(x,y)\mid x\in E, y\in F}>0$. Then $m^*(E\cup F)=m^*(E)+m^*(F)$.
\end{lem}
...
We conclude this section with some useful equivalents of Lebesgue measurability, similar to the ones we developed for Jordan measurability.

\begin{lem}
  The set $E$ is Lebesgue measurable if and only if either of the following hold:
  \begin{enumerate}
  \item For all $\epsilon>0$ there exists an open set $O$ such that $m^*(O\triangle E)<\epsilon$.
  \item For all $\epsilon>0$ there exists a measurable set $E^\epsilon$ such that $m^*(E\triangle E^\epsilon)<\epsilon$.
  \end{enumerate}
\end{lem}

\begin{proof}
  If $E$ is Lebesgue measurable then the definition gives an open set $O$ satisfying the requirements of (a). And since open sets are measurable, (a) clearly implies (b).

  Now let $E$ be a set and assume that (b) holds. Given the set $E_\epsilon$ be as described in (b), let $E'_\epsilon=\bigcup E_{\epsilon/2^k}$. Then it is not difficult to check that $m^*(E'_\epsilon\setminus E)\leq\epsilon$, and $m^*(E\setminus E'\epsilon)=0$. Finally letting $F=\bigcap_nE'_{1/n}$ we have that $F$ is a measurable set and both $m^*(E\setminus F)=0$ and $m^*(F\smallsetminus E)=0$. We leave it as an exercise to check that if $E$ differs from a measurable set by a null set, then $E$ is measurable too.
\end{proof}
