...
In order to define integration with respect t
...
\begin{proof}
  ...
  The same is true of property (e), but we neglected to include it earlier so let us provide the proof now.

  The proof of additivity is similar to the Lebesgue case, but
\end{proof}
...
Once again, if $(X,\mathcal B)$ is a measurable space and $\mu$ is a measure on it, we can define $L^1(\mu)$ to be the space of measurable complex-valued functions on $X$, where two such functions are identified if they agree $\mu$-almost everywhere.
