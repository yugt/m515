...
\begin{defn}
  Let $X$ be a vector space (real or complex). A \emph{norm} on $X$ is a mapping $\|\cdot\|\colon X\to[0,\infty)$ satisfying:
  \begin{itemize}
...
\]
The property means that there are no holes in the space---any point which can be approximated actually exists.

\begin{defn}
  A vector space $X$ equipped with a norm $\|\cdot\|$ is called a \emph{Banach space} if the associated metric is complete.
\end{defn}

We now describe several familiar examples of Banach spaces.
