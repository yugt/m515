...
* Ex 9.2 (see Tao, ex 1.3.1). Show that the simple integral satisfies the properties:
  * (finiteness) $\text{s}\int f\<\infty$ if and only if $f$ is finite almost everywhere and supported on a set of finite measure
  * (vanishing) $\text{s}\int f=0$ if and only if $f=0$ almost everywhere
  * (normality) $\text{s}\int\chi_A=m(A)$ for any Lebesgue measurable set $A$
...
* Ex 7.3 (Tao, ex 1.2.15). Show the \emph{inner regularity} property: If $A$ is Lebesgue measurable, then m(A)=\sup\\{m(K)\mid K\subset A,\text{ $K$ compact}\\}
...
* Ex 3.5. Show that the outer Jordan measure $m^{*j}(A)$ is equal to:   $\inf\\{\text{vol}(B\_1)+\cdots+\text{vol}(B\_k) : B\_1,\ldots,B\_k\text{ are boxes and }A\subset B\_1\cup\cdots\cup B\_k\\}$.
